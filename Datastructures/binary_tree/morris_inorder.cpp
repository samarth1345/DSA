...
//as we do in inorder traversal(left->root->right),we have to keep moving right until the last 
...
vector<int> morris(treeNode<int> *root)
{
    ...
    while(curr)
    {
        ...
        else
        {
            ...
            //if next->right is NULL,we have to make the link to comeback
            if(! next->right)
            {
                next->right=curr;
                curr=curr->left;
            }
            //else we have to break the connection and just push subtree ke parent ka data 
            //into vector and also move to right subtree now as left subtree has been
            //traversed
            else
            {
                next->right=NULL;
